 ...
 */
public interface TrellaRepository extends PagingAndSortingRepository<TrellaEntity, UUID> {

    @Query("SELECT t FROM TrellaEntity t WHERE t.tenantId = :tenantId " +
            "AND LOWER(t.searchText) LIKE LOWER(CONCAT(:textSearch, '%'))")
   
    //TrellaEntity findTrellaById(UUID tenantId, TrellaId trellaId);

    Page<TrellaEntity> findByTenantId(@Param("tenantId") UUID tenantId,
